...
/// project.yml. The settings path stays literal (`~/.config/shotnote/settings.json`): it is
...
enum Identity {
    static let bundleID = Bundle.main.bundleIdentifier ?? "com.petepetrash.shotnote"
    static let name = Bundle.main.infoDictionary?["CFBundleName"] as? String ?? "Shotnote"
    static let urlScheme = urlScheme(in: Bundle.main.infoDictionary ?? [:]) ?? "shotnote"
