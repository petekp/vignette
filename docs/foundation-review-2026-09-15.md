# Vignette foundation review, 2026-09-15
...
Scope: read-only review of AGENTS.md, README.md, project.yml, scripts/, Sources/, web/src/, and the six-commit history, then verification against the running app (same process throughout, PID 95957). Driven with `open -g vignette://…` URLs, `~/Library/Logs/Vignette.log`, `vignette://state`, and screenshots cropped with sips. Four test screenshots were created in the watch folder; three were trashed through the app's own trash action, one was deleted with `rm`. `git status` stayed clean. One ad-hoc-signed build was made into the session scratchpad.

Machine facts that matter to the findings: macOS 15.7.3, Xcode 26.3, xcodegen 2.46.0, tldraw 5.4.2. Two displays: the built-in Retina display is `NSScreen.screens[0]`, the Studio Display sits above it. No python3 on this Mac has pyobjc. Accessibility is granted to `com.petepetrash.vignette`.
...
- Evidence: AnnotationController.swift:75-85, ThumbnailController.swift:260-282. Ran: stack open, annotate test-1, inject a rectangle by `eval`, then two `open -g vignette://annotate` 24 ms apart (log 11:52:27.277 and .301). State: `current=test-review-3 windowVisible=true`, page `images:0`. Screenshot crop showed a blank editor with only the tldraw badge. Script in the appendix.
...
- Consequence: a clone fails at signing. A fork also shares the bundle id, the `vignette` URL scheme, the status item autosave name, and the settings directory, so it collides with the original on the same Mac.
...
- Finding: every placement uses `NSScreen.main`, which is the primary display whenever Vignette has no key window.
...
- Evidence: AppDelegate.swift:165-170. Ran: `open -g "vignette://annotate?file=/tmp/nonexistent%25100.png"`. The `[url]` line shows `%25100` unchanged, so `open` did not re-encode. The `[annotate]` line shows `nonexistent0.png` with byte 0x10 in the name.
...
### 11. `vignette://state` is not machine-readable, is incomplete, and has no end marker
...
- Fix: `vignette://help` generated from `Config.actions` plus the fixed command table. Every command ends with one `[<cmd>] ok …` or `[<cmd>] error …` line. Log the watcher failure.

### 13. Plain `open` activates Vignette; `open -g` does not; the docs never say
- Evidence: Ran `open vignette://recent` then `dismiss`: `[focus] stack dismissed: returned to Ghostty`. Same with `open -g`: no focus line, frontmost app unchanged.
...
- Fix, smallest: expose draft keys in state and log parks now. Later: persist snapshots as JSON under `~/.config/vignette/drafts/` with the image referenced by path, served by the loopback server instead of inlined.
...
- Evidence: Settings.swift:36-87 versus ~/.config/vignette/settings.json on this Mac: cardMaxHeight 150 vs 86, cardMinSide 56 vs 114, annotationMinWidth 480 vs 770, backdropBands 6 vs 3, backdropBlurRadius 40 vs 13, slideInDuration 0.4 vs 0.75, backdropTint 0.3 vs 0. Hard-coded numbers remain at StackView.swift:23-25, 133, 180, 193, 205-215, 227, 263-264, 283, 295, 301; AnnotatorToolbar.swift:22, 61-77, 85-97; TransitionLayer.swift:102; Stitch.swift:5-6, 39, 44.
...
- An xcodegen `VignetteTests` unit-test target, run with `xcodebuild test -scheme Vignette -destination 'platform=macOS'`, added to build.sh behind a flag.
...
- Consequence: `vignette://trash?file=~/Documents/x` works from a browser that remembers the consent.
...
- Evidence: the URL types are at project.yml:22-23 and the bundle id at 27, not 15-17. scripts/run.sh:6-7 sends SIGTERM and opens the new build at once; a guard that quits the newer instance can leave nothing running. README:57 and 86 document `~/.config/vignette/settings.json`.
...
### R10. Finding 13: `open -g` still activates Vignette when the annotator opens
- Evidence: AnnotationController.swift:69; today's log shows `[focus] annotator closed: returned to Notion` after `open -g vignette://cancel`.
...
- [x] 19. [S] Render test and tldraw upgrade checklist (G12; R18). An XCTest starts LocalServer, loads the built page into a WKWebView over the loopback origin, calls `render` with a fixture image and snapshot, and checks non-blank pixels where the annotation is. A checklist in AGENTS.md for bumping tldraw: license state, watermark text, snapshot migrations, the asset store hook, the blank-export workaround, protocol version. Verify: the test fails when `render` is replaced by tldraw's `toImage`. Done 2026-09-15. `Tests/RenderTests.swift` starts LocalServer on the test bundle's copy of `web/dist` (a resource of VignetteTests in project.yml), loads the page into a WKWebView, checks `ready` carries the app's protocol, loads a 400x300 solid blue fixture, draws a red solid rectangle over its middle through `window.editor`, exports through `window.vignette.export`, and checks the PNG is 400x300, the corner is the fixture's blue, the middle is red over blue, and `getCanUndo()` is still true. With `render` swapped for `editor.toImage` the test fails at once (an 800-pixel export where 400 was expected), and passes again restored. Two things learned: the WKWebView in a test is never on screen, so `loaded` (a requestAnimationFrame) never arrives and the test polls for the image shape instead; canvas drawing is color managed, so the fixture's blue comes back shifted by about 40 units per channel and the checks are tolerances, not equality. The checklist is in AGENTS.md under the tldraw bullet. Deviation: no unit test for the checklist itself; the render test is its gate.

### Phase C: ship

- [x] 20. [B] Launch at login (new). `SMAppService` behind a setting, offered once by the first-launch toast. Verify: `sfltool dumpbtm` lists the app after enabling; turning the setting off removes it. Done 2026-09-15. `launchAtLogin` in settings.json, off by default; `LoginItem.apply` registers or unregisters through `SMAppService.mainApp` at launch and on every change, logging `[login] registered|unregistered|error`; a Startup section in the settings window holds the toggle; `[state] app` carries the setting and macOS's status. Verified with the scratch settings: turning the key on in the file gave `[login] registered status=enabled` within a second and `sfltool dumpbtm` listed Vignette as `[enabled, allowed, visible, notified]` at the build path; turning it off gave `unregistered status=notRegistered`, and the record stayed in the database marked `disabled`, which is how macOS records an unregistered item, so "removes it" reads as "disables it" there. A first launch against a missing settings path showed the toast "Vignette is watching ~/Dropbox/Screenshots. Launch at login is off; turn it on in Settings." Deviation: the toast points at Settings rather than offering a button, since toasts take no input. Note for anyone verifying: `sfltool dumpbtm` asks for the user's password on every call.
- [x] 21. [S] Docs (finding 25; R10). Fix every line in finding 25; use `open -g` everywhere and say that opening the annotator activates Vignette regardless; describe `scripts/input.sh`, the coordinate convention, the log grammar, the error codes, the `debug` setting, the settings override, the license key, `restore-apple-defaults`, and a fork checklist (rename in project.yml, `signing.env` or a self-signed certificate, `web/dist` before `xcodegen`, drafts on disk survive relaunch). Verify: each README command exercised once against the built app; the AGENTS.md dev loop followed end to end on a fresh clone. Done 2026-09-15. README rewritten: the bridge paragraph, the loopback server with its token and same-origin reason, drafts on disk, `open -g` everywhere with the note that the annotator activates Vignette, every command from `vignette://help` with the error codes and the log grammar, `scripts/input.sh` and the top-left coordinate convention, the `debug` setting, `VIGNETTE_SETTINGS`, the license key, `restore-apple-defaults`, `launchAtLogin`, the tuned `ui` defaults, the Accessibility requirement for double-rshift, the signing section kept with its unverified self-signed claim, and a fork checklist. The vite.config.ts comment no longer mentions file://; AGENTS.md dropped `ExpandPanel` and the stale `imageUrl` sentence. Verified: every README command answered with an ok line against the built app on the scratch settings (test files and the stitch output removed through `trash`; `restore-apple-defaults` wrote values equal to the current ones); a fresh clone in the scratchpad built and passed the 99 tests ad-hoc signed with no `signing.env`, and its app launched, answered `state`, and was replaced by this checkout's build. Noted, not fixed: `cancel` does not close the `show-editor` window, since that window has no image; a click outside the annotator closes it, so an annotator opened while a person is using the Mac may be gone before the next command.

Deferred, not steps: a control socket and a `vignette` CLI that returns JSON instead of log polling (decide after step 17); FSEvents, if a folder replacement under Dropbox is ever observed; a Theme enum for the remaining constants.
...
```sh
W=$HOME/Dropbox/Screenshots; LOG=$HOME/Library/Logs/Vignette.log
...
sleep 2.5
open -g vignette://recent; sleep 1.2
open -g "vignette://annotate?file=$(enc "$W/Screenshot test-review-1.png")"; sleep 1.6
...
open -g "vignette://eval?$(python3 -c 'import urllib.parse,sys; print(urllib.parse.quote(sys.argv[1], safe=""))' "$JS")"; sleep 0.8
open -g "vignette://annotate?file=$(enc "$W/Screenshot test-review-2.png")" & open -g "vignette://annotate?file=$(enc "$W/Screenshot test-review-3.png")" & wait
sleep 2.2
open -g vignette://state; sleep 0.8
sed -n "$((m+1)),\$p" "$LOG" | cut -c1-420
# Before the fix: annotator current=test-review-3 windowVisible=true and page state images:0.
open -g vignette://cancel; sleep 1.3; open -g vignette://dismiss; sleep 1.6
```
...
```sh
m=$(wc -l < "$LOG")
open -g "vignette://annotate?file=$(enc "$W/Screenshot test-review-1.png")"; sleep 2.0
screencapture -x -R 200,200,600,400 "$W/Screenshot test-review-4.png"; sleep 1.6
open -g vignette://state; sleep 0.7
...
```sh
m=$(wc -l < "$LOG")
open -g vignette://recent; sleep 1.3
rm "$W/Screenshot test-review-4.png"; sleep 0.6
open -g "vignette://annotate?file=$(enc "$W/Screenshot test-review-4.png")"; sleep 1.9
open -g vignette://state; sleep 0.7
sed -n "$((m+1)),\$p" "$LOG" | cut -c1-300
...
```sh
open -g "vignette://trash?file=$(enc "$W/Screenshot test-review-1.png")&file=$(enc "$W/Screenshot test-review-2.png")&file=$(enc "$W/Screenshot test-review-3.png")"
...
```sh
m=$(wc -l < "$LOG")
open -g "vignette://annotate?file=/tmp/nonexistent%20a%20b.png"; sleep 0.4
open -g "vignette://annotate?file=/tmp/nonexistent%25100.png"; sleep 0.4
sed -n "$((m+1)),\$p" "$LOG"
...
```sh
open -g vignette://recent; sleep 1; open -g vignette://dismiss; sleep 1.3   # expect no [focus] line
open vignette://recent; sleep 1; open vignette://dismiss; sleep 1.3         # expect "[focus] stack dismissed: returned to …"
```

### Ad-hoc build (finding 7, step 8)

```sh
xcodebuild -project Vignette.xcodeproj -scheme Vignette -configuration Debug \
  -derivedDataPath /tmp/vignette-adhoc build -quiet CODE_SIGN_IDENTITY=- CODE_SIGN_STYLE=Manual DEVELOPMENT_TEAM=
codesign -dv /tmp/vignette-adhoc/Build/Products/Debug/Vignette.app 2>&1 | grep -E 'Signature|TeamIdentifier'
