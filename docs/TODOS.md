...
- Vendor logos on the agent badge: Claude's is in (`Resources/agents/claude.svg`, Pete's call on
  2026-09-20, the badge is a white "From Claude" tab); ChatGPT's and others wait on the same review.
  One `<name>.svg` in that folder per `agent=` value is all `Agent.logo(for:)` needs.
